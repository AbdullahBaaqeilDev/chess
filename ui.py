...
class UIController:
    def __init__(
        self, engine: ChessEngine | None = None, audio: AudioController | None = None
    ):
        pygame.init()
        pygame.mixer.init()
        ...
        self.legal_moves_surf = pygame.Surface(
            (8 * SQ_SIZE, 8 * SQ_SIZE), flags=pygame.SRCALPHA
        )
    ...
    def load_images(self):
        ...
        for p in pieces:
            ...
            if os.path.exists(path):
                ...
                self.images[p[-1].upper() if p[0] == "w" else p[-1]] = (
                    pygame.transform.smoothscale(img, (SQ_SIZE, SQ_SIZE))
                )
        ...
        if self.selected_sq:
            ...
            if move:
                if move.flags & MoveFlag.PROMOTION:
                    move.promotion_choice = self.show_promotion_ui(
                        move.piece_moved.isupper()
                    )
